...
def remove_whitespace(string):
    ...
    code_blocks_mapping = set()
    for code_block in code_blocks:
        code_hash = str(hash(code_block))
        string = string.replace(code_block, code_hash)
        code_blocks_mapping.add((code_hash, code_block))

    string = re.sub(r'\s*\n\s*', ' ', string)
    string = re.sub(r'>\s{2+}<', '><', string)

    for code_hash, code_block in code_blocks_mapping:
        string = string.replace(code_hash, code_block)
